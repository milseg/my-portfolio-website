...
type LinkOrAnchorProps = React.DetailedHTMLProps<React.AnchorHTMLAttributes<HTMLAnchorElement>, HTMLAnchorElement> | {
  children: React.ReactNode;
  href: string;
  className: string;
};

const LinkOrAnchor = ({children, ...props}: LinkOrAnchorProps) => {
  const l = props.href as string
  if(l.startsWith('/')) {
    return (
      <Link to={l} className={props.className} >
        {children}
      </Link>
    )
  } else {
        ...
        <div className="grid grid-cols-3 gap-4">
          {
            data.allMdx?.nodes?.map((node) => {
              return (
                <LinkOrAnchor href={node.frontmatter?.guru as string} className="relative group overflow-hidden rounded cursor-pointer" key={node.id} title={node.frontmatter?.title as string} target="_blank" rel="noopener noreferrer">
